#!/usr/bin/env python3
"""
SAS/SNDS conventions hook (PreToolUse, matcher Edit|Write).

Fires whenever a .sas file is about to be edited/written and injects the
SAS/SNDS coding rules into context, so the conventions in
rules/sas-sql-conventions.md are ACTUALLY APPLIED, not just documented.

Output: JSON with hookSpecificOutput.additionalContext on a .sas target;
...
RULES = """SAS/SNDS coding rules -- APPLY NOW before editing this .sas file (rules/sas-sql-conventions.md):
