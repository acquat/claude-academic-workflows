...
**The cycle:** implement → **verify** (compile / run / open the actual output) → **review** (run the reviewer agent that matches the file type) → **fix** what review found, worst first → verify again → **score** against [quality-gates.md](quality-gates.md).
